...
namespace GlbGlobe
{
	...
	// return (Cx, Cy, r^2)
	inline osg::Vec3d compute_circumcircle(
		const osg::Vec3d &a,
		const osg::Vec3d &b,
		const osg::Vec3d &c)
	{
	...
	public:
		...
		Triangle():
		  ...
		  Triangle(Vertex_index a, Vertex_index b, Vertex_index c, osg::Vec3dArray *points)
			  :    a_(a),
			  b_(b),
			  c_(c),
		  ...
		  inline const osg::Vec3d &get_circumcircle() const { return cc_; }

		  inline osg::Vec3d compute_centroid(const osg::Vec3dArray *points) const
		  {
			  return ((*points)[a_] +(*points)[b_] + (*points)[c_])/3;
		  }

		  inline osg::Vec3d compute_normal(osg::Vec3dArray *points) const
		  {
			  osg::Vec3d N = ((*points)[b_] - (*points)[a_]) ^ ((*points)[c_] - (*points)[a_]);
			  return N / N.length();
		  }
		  ...
		  bool intersected(const unsigned int ip1,const unsigned int ip2,const osg::Vec2d p1 ,const osg::Vec2d p2,const int iedge, osg::Vec3dArray *points) const
		  {
			  ...
			  osg::Vec2d tp1((*points)[ie1].x(),(*points)[ie1].y());
			  osg::Vec2d tp2((*points)[ie2].x(),(*points)[ie2].y());
			  return intersect(tp1,tp2,p1,p2);
		  }

		  bool intersectedby(const osg::Vec2d p1,const osg::Vec2d p2,osg::Vec3dArray *points) const {
			  // true if line [p1,p2] cuts at least one edge of this triangle
			  osg::Vec2d tp1((*points)[a()].x(),(*points)[a()].y());
			  osg::Vec2d tp2((*points)[b()].x(),(*points)[b()].y());
			  osg::Vec2d tp3((*points)[c()].x(),(*points)[c()].y());
		  ...
		  int whichEdge(osg::Vec3dArray *points,const osg::Vec2d p1, const osg::Vec2d p2,
			  const unsigned int e1,const unsigned int e2) const
		  {
			  ...
			  osg::Vec2d tp1((*points)[a()].x(),(*points)[a()].y()); // triangle vertices
			  osg::Vec2d tp2((*points)[b()].x(),(*points)[b()].y());
			  osg::Vec2d tp3((*points)[c()].x(),(*points)[c()].y());
		  ...
		  int lineBisectTest(const osg::Vec3d apt,const osg::Vec3d bpt,const osg::Vec3d cpt, const osg::Vec2d p2) const
		  {
			  osg::Vec2d vp2tp=p2-osg::Vec2d(apt.x(), apt.y()); // vector from p1 to a.
			  // test is: cross product (z component) with ab,ac is opposite signs
			  // AND dot product with ab,ac has at least one positive value.
			  osg::Vec2d vecba=osg::Vec2d(bpt.x(), bpt.y())-osg::Vec2d(apt.x(), apt.y());
			  osg::Vec2d vecca=osg::Vec2d(cpt.x(), cpt.y())-osg::Vec2d(apt.x(), apt.y());
			  double cprodzba=vp2tp.x()*vecba.y() - vp2tp.y()*vecba.x();
			  double cprodzca=vp2tp.x()*vecca.y() - vp2tp.y()*vecca.x();
			  ...
			  if (cprodzba*cprodzca<0)
			  {
				  ...
				  osg::Vec2d tp1(bpt.x(),bpt.y()); // triangle vertices
				  osg::Vec2d tp2(cpt.x(),cpt.y());
				  osg::Vec2d tp3(apt.x(),apt.y());
		  ...
		  int lineBisects(osg::Vec3dArray *points, const unsigned int ip1, const osg::Vec2d p2) const
		  {
	...
	private:
		...
		bool intersect(const osg::Vec2d p1,const osg::Vec2d p2,const osg::Vec2d p3,const osg::Vec2d p4) const
		{
	...
	bool Sample_point_compare(const osg::Vec3d &p1, const osg::Vec3d &p2)
	{
	...
	DelaunayTriangulatorEx::DelaunayTriangulatorEx():
	osg::Referenced()
	{
	}

	DelaunayTriangulatorEx::DelaunayTriangulatorEx(osg::Vec3dArray *points, osg::Vec3dArray *normals):
	osg::Referenced(),
		points_(points),
		normals_(normals)
	{
	...
	DelaunayTriangulatorEx::DelaunayTriangulatorEx(const DelaunayTriangulatorEx &copy, const osg::CopyOp &copyop):
	osg::Referenced(copy),
		points_(static_cast<osg::Vec3dArray *>(copyop(copy.points_.get()))),
		normals_(static_cast<osg::Vec3dArray *>(copyop(copy.normals_.get()))),
	...
	int DelaunayTriangulatorEx::getindex(const osg::Vec3d &pt,const osg::Vec3dArray *points)
	{
	...
	Triangle_list fillHole(osg::Vec3dArray *points,    std::vector<unsigned int> const &vindexlist)
	{
		...
		osg::ref_ptr<osg::Vec3dArray> constraintverts=new osg::Vec3dArray;
		osg::ref_ptr<osgUtil::Tessellator> tscx=new osgUtil::Tessellator; // this assembles all the constraints
		//osg::ref_ptr<GlbGlobe::TessellatorEx> tscx=new GlbGlobe::TessellatorEx; // this assembles all the constraints

		for (std::vector<unsigned int>::const_iterator itint=vindexlist.begin(); itint!=vindexlist.end(); itint++)
		{
		...
		//tscx->setWindingType( GlbGlobe::TessellatorEx::TESS_WINDING_ODD); // the commonest tessellation is default, ODD. GE2 allows intersections of constraints to be found.
	...
	{    /** remove vertices from this which are internal to dco.
		 ...
		 */
		int nrem=0;
		osg::Vec3dArray *vertices= dynamic_cast< osg::Vec3dArray*>(getVertexArray());
		if (vertices)
		{
			for (osg::Vec3dArray::iterator vitr=vertices->begin(); vitr!=vertices->end(); )
			{
	...
	void DelaunayConstraint::merge(DelaunayConstraint *dco)
	{
		...
		if (dco) { // 16 Dec 2006 just in case you pass in a NULL pointer
			osg::Vec3dArray* vmerge=dynamic_cast<osg::Vec3dArray*>(getVertexArray());
			if (!vmerge) vmerge=new osg::Vec3dArray;
			...
			osg::Vec3dArray* varr=dynamic_cast<osg::Vec3dArray*>(dco->getVertexArray());
			if (varr) vmerge->insert(vmerge->end(),varr->begin(),varr->end());
		}
	}

	void DelaunayTriangulatorEx::_uniqueifyPoints()
	{
		std::sort( points_->begin(), points_->end() );

		osg::ref_ptr<osg::Vec3dArray> temppts = new osg::Vec3dArray;
		...
		osg::Vec3dArray::iterator p = points_->begin();
		osg::Vec3d v = *p;
		...
		points_->clear();
		std::insert_iterator< osg::Vec3dArray > ci(*(points_.get()),points_->begin());
		std::copy( temppts->begin(), temppts->end(), ci );
	}

	GlbGlobe::DelaunayConstraint *getconvexhull(osg::Vec3dArray *points)
	{ // fits the 'rubberband' around the 2D points for uses as a delaunay constraint.
		osg::ref_ptr<GlbGlobe::DelaunayConstraint> dcconvexhull=new GlbGlobe::DelaunayConstraint; // make
		// convex hull around all the points
		// start from first point (at minx); proceed to last x and back
		osg::Vec3dArray *verts=new osg::Vec3dArray; // the hull points
		...
		for (osg::Vec3dArray::iterator vit=(points->begin()+2); vit!=points->end(); vit++) {
			// check if point lies outside the current last line segment
			bool ok=1;
			while (ok && verts->size()>1) {
				osg::Vec3d lastseg=*(verts->end()-2)-*(verts->end()-1);
				osg::Vec3d thisseg=(*vit)-*(verts->end()-1);
				double cosang=(lastseg^thisseg).z();
		...
		for (osg::Vec3dArray::reverse_iterator rvit=points->rbegin()+1; rvit!=points->rend(); rvit++) {
			// check if point lies outside the current last line segment
			bool ok=1;
			while (ok && verts->size()>1) {
				osg::Vec3d lastseg=*(verts->end()-2)-*(verts->end()-1);
				osg::Vec3d thisseg=(*rvit)-*(verts->end()-1);
				double cosang=(lastseg^thisseg).z();
	...
	bool DelaunayTriangulatorEx::triangulate()
	{
		...
		osg::Vec3dArray *points = points_.get();
		...
		for (linitr=constraint_lines.begin();linitr!=constraint_lines.end();linitr++)
		{
			...
			const osg::Vec3dArray* vercon= dynamic_cast<const osg::Vec3dArray*>(dc->getVertexArray());
			if (vercon)
			{
				int nadded=0;
				for (unsigned int icon=0;icon<vercon->size();icon++)
				{
					osg::Vec3d p1=(*vercon)[icon];
		...
		osg::ref_ptr<GlbGlobe::DelaunayConstraint> dcconvexhull=getconvexhull(points);
		...
		double maxx = (*points)[last_valid_index].x();
		...
		osg::Vec3dArray::const_iterator mmi;
		...
		points_->push_back(osg::Vec3d(minx - .10*(maxx - minx), miny - .10*(maxy - miny), 0));
		points_->push_back(osg::Vec3d(maxx + .10*(maxx - minx), miny - .10*(maxy - miny), 0));
		points_->push_back(osg::Vec3d(maxx + .10*(maxx - minx), maxy + .10*(maxy - miny), 0));
		points_->push_back(osg::Vec3d(minx - .10*(maxx - minx), maxy + .10*(maxy - miny), 0));
		...
		// begin triangulation
		GLuint pidx = 0;
		osg::Vec3dArray::const_iterator i;
		...
		for (i=points->begin(); i!=points->end(); ++i, ++pidx)
		{
			...
			for (j=triangles.begin(); j!=triangles.end(); j = next_j)
			{
				...
				osg::Vec3d cc = j->get_circumcircle();
		...
		for (linelist::iterator dcitr=constraint_lines.begin();dcitr!=constraint_lines.end();dcitr++)
		{
			...
			const osg::Vec3dArray* vercon = dynamic_cast<const osg::Vec3dArray*>((*dcitr)->getVertexArray());
			if (vercon)
			{
						...
						for (unsigned int i=0; i<prset->getNumIndices(); i++)
						{
							...
							if (i>0 || prset->getMode()==osg::PrimitiveSet::LINE_LOOP)
							{
								...
								if (!edgused)
								{
									...
									osg::Vec2d p1((*points_)[ip1].x(),(*points_)[ip1].y()); // a constraint line joins p1-p2
									osg::Vec2d p2((*points_)[ip2].x(),(*points_)[ip2].y());
									int ntr=0;
	...
	void DelaunayTriangulatorEx::removeInternalTriangles(DelaunayConstraint *dc )
	{
		if (dc) { // 16.12.06 just in case....
			...
			int ndel=0;
			osg::Vec3dArray::iterator normitr;
			if( normals_.valid() )
	...
	double DelaunayConstraint::windingNumber(const osg::Vec3d &testpoint) const
	{
		// return winding number of loop around testpoint. Only in 2D, x-y coordinates assumed!
		double theta=0; // sum of angles subtended by the line array - the winding number
		const osg::Vec3dArray *vertices= dynamic_cast<const osg::Vec3dArray*>(getVertexArray());
		if (vertices)
		{
			for (unsigned int ipr=0; ipr<getNumPrimitiveSets(); ipr++)
			{
				...
				if (prset->getMode()==osg::PrimitiveSet::LINE_LOOP)
				{
					// nothing else loops
					// start with the last point on the loop
					const osg::Vec3d prev=(*vertices)[prset->index (prset->getNumIndices()-1)];
					osg::Vec3d pi(prev.x()-testpoint.x(),prev.y()-testpoint.y(),0);
					pi.normalize();
					for (unsigned int i=0; i<prset->getNumIndices(); i++)
					{
						const osg::Vec3d curp=(*vertices)[prset->index (i)];
						osg::Vec3d edge(curp.x()-testpoint.x(),curp.y()-testpoint.y(),0);
						edge.normalize();
						double cth=edge*pi;
						if (cth<=-0.99999 )
						{
						...
						else
						{
							if (cth<0.99999)
							{
								double dang=(cth<1 && cth>-1)?acos(edge*pi):0; // this is unsigned angle
								double zsign=edge.x()*pi.y()-pi.x()*edge.y(); // z component of..(edge^pi).z();
	...
	bool DelaunayConstraint::contains(const osg::Vec3d &testpoint) const
	{
		// true if point is internal to the loop.
		double theta=windingNumber(testpoint); // sum of angles subtended by the line array - the winding number
		return fabs(theta)>0.9; // should be 0 or 1 (or 2,3,4 for very complex not permitted loops).
	}
	bool DelaunayConstraint::outside(const osg::Vec3d &testpoint) const
	{
	...
	osg::Vec3dArray* DelaunayConstraint::getPoints(const osg::Vec3dArray *points)
	{
		//points_ is the array of points that can be used to render the triangles in this DC.
		osg::ref_ptr<osg::Vec3dArray> points_=new osg::Vec3dArray;
		trilist::iterator ti;
		for (ti=_interiorTris.begin(); ti!=_interiorTris.end(); ++ti) {
			int idx=0;
			int ip[3]={-1,-1,-1};
			// find if points[i1/i2/i3] already in the vertices points_
			for (osg::Vec3dArray::iterator ivert=points_->begin(); ivert!=points_->end(); ivert++)
			{
