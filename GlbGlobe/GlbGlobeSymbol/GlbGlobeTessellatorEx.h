...
#ifndef OSGUTIL_TessellatorEX
#define OSGUTIL_TessellatorEX
...
namespace GlbGlobe 
{
	...
	* Upgraded Jan 2004 to use the modern glu tessellation functions.*/
	...
	class TessellatorEx : public osg::Referenced
	{
	public:
		...
		* list of primitives in the original geometry is stored in the TessellatorEx for
		* possible re-use.
		...
		*/
		...
		struct Prim : public osg::Referenced
		{
			...
			typedef std::vector<osg::Vec3d *> VecList;
	...
	protected:
		...
		* these will leak memory if not removed when retessellating. */
		...
		typedef std::map<osg::Vec3d *,unsigned int> VertexPtrToIndexMap;
		...
		void vertex(osg::Vec3d *vertex);
		void combine(osg::Vec3d *vertex,void* vertex_data[4],GLdouble weight[4]);
		void end();
		...
		NewVertex(osg::Vec3d* vx,
			double f1,osg::Vec3d* v1,
			double f2,osg::Vec3d* v2,
			double f3,osg::Vec3d* v3,
			double f4,osg::Vec3d* v4):
		_vpos(vx),
			_f1(f1),
			_v1(v1),
			_f2(f2),
		...
		osg::Vec3d  *_vpos; // added gwm Jan 2004 the vertex coords s.t. NewVertex can be used in a std::vector
