...
void TessellatorEx::beginTessellation()
{
	reset();
...
void TessellatorEx::beginContour()
{
	if (_tobj)
	{
...
void TessellatorEx::addVertex(osg::Vec3d* vertex)
{
	if (_tobj)
	{
		if (vertex && vertex->valid())
		{
			TessellatorEx::Vec3d* data = new TessellatorEx::Vec3d;
		...
		{
			OSG_INFO<<"TessellatorEx::addVertex("<<*vertex<<") detected NaN, ignoring vertex."<<std::endl;
		}
	}
}

void TessellatorEx::endContour()
{
	if (_tobj)
	{
...
void TessellatorEx::endTessellation()
{
	if (_tobj)
	{
...
public:
	...
	InsertNewVertices(double f1,unsigned int i1,
		double f2,unsigned int i2,
		double f3,unsigned int i3,
		double f4,unsigned int i4):
	...
	virtual void apply(osg::FloatArray& ba) { apply_imp(ba,float(0.0f)); }
	...
	virtual void apply(osg::DoubleArray& ba) { apply_imp(ba,double(0.0)); }
...
void TessellatorEx::retessellatePolygons(osg::Geometry &geom)
{
	// turn the contour list into primitives, a little like TessellatorEx does but more generally
	osg::Vec3dArray* vertices = dynamic_cast<osg::Vec3dArray*>(geom.getVertexArray());

	if (!vertices || vertices->empty() || geom.getPrimitiveSetList().empty()) return;

	if (_ttype==TESS_TYPE_POLYGONS || _ttype==TESS_TYPE_DRAWABLE) _numberVerts=0; // 09.04.04 GWM reset TessellatorEx
	// the reset is needed by the flt loader which reuses a TessellatorEx for triangulating polygons.
	...
	// the main difference from osgUtil::TessellatorEx for Geometry sets of multiple contours is that the begin/end tessellation
	// occurs around the whole set of contours.
	if (_ttype==TESS_TYPE_GEOMETRY) {
		beginTessellation();
	}
	// process all the contours into the TessellatorEx
...
void TessellatorEx::addContour(GLenum mode, unsigned int first, unsigned int last, osg::Vec3dArray* vertices)
{
	beginContour();
...
void TessellatorEx::addContour(osg::PrimitiveSet* primitive, osg::Vec3dArray* vertices)
{
	...
	default:
		OSG_NOTICE<<"TessellatorEx::addContour(primitive, vertices) : Primitive type "<<primitive->getType()<<" not handled"<<std::endl;
		break;
	}

}

void TessellatorEx::handleNewVertices(osg::Geometry& geom,VertexPtrToIndexMap &vertexPtrToIndexMap)
{
	if (!_newVertexList.empty())
	{

		osg::Vec3dArray* vertices = dynamic_cast<osg::Vec3dArray*>(geom.getVertexArray());
		osg::Vec3dArray* normals = NULL;
		if (osg::getBinding(geom.getNormalArray())==osg::Array::BIND_PER_VERTEX)
		{
			normals = dynamic_cast<osg::Vec3dArray*>(geom.getNormalArray());
		}
			...
			++itr)
		{
			...
			osg::Vec3d* vertex = newVertex._vpos;
...
void TessellatorEx::begin(GLenum mode)
{
	_primList.push_back(new Prim(mode));
}

void TessellatorEx::vertex(osg::Vec3d* vertex)
{
	if (!_primList.empty())
	{
...
void TessellatorEx::combine(osg::Vec3d * vertex,void* vertex_data[4],GLdouble weight[4])
{
...
void TessellatorEx::end()
{
	// no need to do anything right now...
}

void TessellatorEx::error(GLenum errorCode)
{
	_errorCode = errorCode;
}

void CALLBACK TessellatorEx::beginCallback(GLenum which, void* userData)
{
	((TessellatorEx*)userData)->begin(which);
}

void CALLBACK TessellatorEx::endCallback(void* userData)
{
	((TessellatorEx*)userData)->end();
}

void CALLBACK TessellatorEx::vertexCallback(GLvoid *data, void* userData)
{
	((TessellatorEx*)userData)->vertex((osg::Vec3d *)data);
}

void CALLBACK TessellatorEx::combineCallback(GLdouble coords[3], void* vertex_data[4],
	GLdouble weight[4], void** outData,
	void* userData)
{
	osg::Vec3d* newData = new osg::Vec3d(coords[0],coords[1],coords[2]);
	*outData = newData;
	((TessellatorEx*)userData)->combine(newData,vertex_data,weight);
}

void CALLBACK TessellatorEx::errorCallback(GLenum errorCode, void* userData)
{
	((TessellatorEx*)userData)->error(errorCode);
}

void TessellatorEx::reduceArray(osg::Array * cold, const unsigned int nnu)
{ // shrinks size of array to N
	if (cold && cold->getNumElements()>nnu) {
		osg::Vec2dArray* v2arr = NULL;
		osg::Vec3dArray* v3arr = NULL;
		osg::Vec4dArray* v4arr = NULL;
		switch (cold->getType()) {
		case osg::Array::Vec2dArrayType: {
			v2arr = dynamic_cast<osg::Vec2dArray*>(cold);
			osg::Vec2dArray::iterator itr=v2arr->begin()+nnu;
			(*v2arr).erase(itr, v2arr->end());
										 }
										 break;
		case osg::Array::Vec3dArrayType: {
			v3arr = dynamic_cast<osg::Vec3dArray*>(cold);
			osg::Vec3dArray::iterator itr=v3arr->begin()+nnu;
			(*v3arr).erase(itr, v3arr->end());
										 }
										 break;
		case osg::Array::Vec4dArrayType: {
			v4arr = dynamic_cast<osg::Vec4dArray*>(cold);
			osg::Vec4dArray::iterator itr=v4arr->begin()+nnu;
...
void TessellatorEx::collectTessellation(osg::Geometry &geom, unsigned int /*originalIndex*/)
{
	...
	{
		osg::Vec3dArray* normals = NULL; // GWM Sep 2002 - add normals for extra facets
		int iprim=0;
		if (osg::getBinding(geom.getNormalArray())==osg::Array::BIND_PER_PRIMITIVE_SET)
		{
			normals = dynamic_cast<osg::Vec3dArray*>(geom.getNormalArray()); // GWM Sep 2002
		}
		// GWM Dec 2003 - needed to add colours for extra facets
		osg::Vec4dArray* cols4 = NULL; // GWM Dec 2003 colours are vec4
		osg::Vec3dArray* cols3 = NULL; // GWM Dec 2003 colours are vec3
		if (osg::getBinding(geom.getColorArray())==osg::Array::BIND_PER_PRIMITIVE_SET)
		{
			...
			case osg::Array::Vec4dArrayType:
				cols4=dynamic_cast<osg::Vec4dArray *> (colours);
				break;
			case osg::Array::Vec3dArrayType:
				cols3=dynamic_cast<osg::Vec3dArray *> (colours);
				break;
			default:
				break;
			}
		...
		osg::Vec3d norm(0.0,0.0,0.0);
		osg::Vec4d primCol4(0.0,0.0,0.0,1.0);
		osg::Vec3d primCol3(0.0,0.0,0.0);

		for(PrimList::iterator primItr=_primList.begin(); primItr!=_primList.end(); ++primItr, ++_index)
		{
